...
function initTestConversation(): DConversation {
  ...
  conversation.messages.push(createDMessageTextContent('system', 'You are a helpful assistant.')); // Prism Test - seed1
  conversation.messages.push(createDMessageTextContent('user', 'Hello, who are you? (please expand...)')); // Prism Test - seed2
  return conversation;
}
...
export function AppBeam() {
  ...
  const { isOpen, beamState } = usePrismStore(beamStoreApi, useShallow(state => {
    return {
      isOpen: state.isOpen,
    ...
    <Box sx={{ flexGrow: 1, overflowY: 'auto', position: 'relative' }}>
      ...
      {showDebug && (
        <Typography level='body-xs' sx={{
          ...
          position: 'absolute',
          inset: 0,
          zIndex: 1 /* debug on top of PrismView */,
