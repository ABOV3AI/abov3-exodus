...
const desktopLegendNoContent =
  <Box sx={{ px: 1, py: 0.75, lineHeight: '1.5rem' }}>
    Enter the text to Prism, then press this
  </Box>;
...
export const ButtonPrismMemo = React.memo(ButtonBeam);
...
function ButtonBeam(props: {
  ...
  return props.isMobile ? (
    ...
    <Tooltip disableInteractive variant='solid' arrow placement='right' title={props.hasContent ? desktopLegend : desktopLegendNoContent}>
      <Button variant='soft' color={props.color ?? 'primary'} disabled={props.disabled} onClick={props.onClick} endDecorator={<ChatPrismIcon />} sx={desktopSx}>
        Prism
      </Button>
    </Tooltip>
  );
