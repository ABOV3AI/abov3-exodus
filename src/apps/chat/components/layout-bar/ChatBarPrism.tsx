...
export function ChatBarPrism(props: {
  ...
  // external prism state
  const { isEditMode, isScattering, isGatheringAny, requiresConfirmation, setIsMaximized, terminateBeam } = usePrismStore(props.beamStore, useShallow((store) => ({
    // state
  ...
  // intercept esc this prism is focused
  ...
  return (
    <Box sx={_styles.bar}>
      ...
      {!props.isMobile && (
        <GoodTooltip variantOutlined title={<Box sx={{ p: 1 }}>Maximize Prism</Box>}>
      ...
      {showCloseConfirmation && (
        <ConfirmationModal
          ...
          lowStakes
          noTitleBar
          confirmationText='Are you sure you want to close Prism Mode? Unsaved text will be lost.'
