...
export function PrismView(props: {
  ...
  const rayIds = usePrismStore(props.beamStore, useShallow(state => state.rays.map(ray => ray.rayId)));
  const fusionIds = usePrismStore(props.beamStore, useShallow(state => state.fusions.map(fusion => fusion.fusionId)));
  ...
  useGlobalShortcuts('PrismView', React.useMemo(() => [
  ...
  return <>
    ...
    <Box role='prism-list' sx={{
      // scroller fill
      minHeight: '100%',
      // ...props.sx,

      // enter animation
      // NOTE: disabled: off-putting/confusing when the prism content is large - things won't combine nicely
