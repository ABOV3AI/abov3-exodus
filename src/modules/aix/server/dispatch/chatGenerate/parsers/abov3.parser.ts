...
import { ABOV3Wire_API_Message_Create } from '../../wiretypes/abov3.wiretypes';


/**
 * ABOV3 Streaming Completions - Messages Architecture
 *
 * ABOV3 uses a events-based, chunk-based streaming protocol for its chat completions:
 ...
 * Server Tools: Tools executed by ABOV3's infrastructure. The model emits `server_tool_use`
 ...
 */
export function createABOV3MessageParser(): ChatGenerateParseFunction {
  const parserCreationTimestamp = Date.now();
  let responseMessage: ABOV3Wire_API_Message_Create.Response;
  ...
  return function(pt: IParticleTransmitter, eventData: string, eventName?: string): void {
    ...
    if (hasErrored)
      console.log('ABOV3 stream has errored already, but received more data:', eventData);
    ...
    switch (eventName) {
      ...
      case 'message_start':
        ...
        if (!isFirstMessage)
          throw new Error('Unexpected second message - we only support 1 ABOV3 message at a time');

        // Throws on malformed event data, or even role != 'assistant'
        responseMessage = ABOV3Wire_API_Message_Create.event_MessageStart_schema.parse(JSON.parse(eventData)).message;
      ...
      case 'content_block_start': {
        ...
        const { index, content_block } = ABOV3Wire_API_Message_Create.event_ContentBlockStart_schema.parse(JSON.parse(eventData));
        ...
        switch (content_block.type) {
          ...
          case 'tool_use':
            // [ABOV3] Note: .input={} and is parsed as an object - if that's the case, we zap it to ''
      ...
      case 'content_block_delta': {
        ...
        const { index, delta } = ABOV3Wire_API_Message_Create.event_ContentBlockDelta_schema.parse(JSON.parse(eventData));
      ...
      case 'content_block_stop': {
        ...
        const { index } = ABOV3Wire_API_Message_Create.event_ContentBlockStop_schema.parse(JSON.parse(eventData));
        if (responseMessage.content[index] === undefined)
      ...
      case 'message_delta':
        if (responseMessage) {
          const { delta, usage } = ABOV3Wire_API_Message_Create.event_MessageDelta_schema.parse(JSON.parse(eventData));

          Object.assign(responseMessage, delta);

          // -> Token Stop Reason
          const tokenStopReason = _fromABOV3StopReason(delta.stop_reason);
      ...
      case 'message_stop':
        ABOV3Wire_API_Message_Create.event_MessageStop_schema.parse(JSON.parse(eventData));
...
export function createABOV3MessageParserNS(): ChatGenerateParseFunction {
  const parserCreationTimestamp = Date.now();
  ...
  return function(pt: IParticleTransmitter, fullData: string): void {
    ...
    const {
      ...
      stop_reason,
      usage,
    } = ABOV3Wire_API_Message_Create.Response_schema.parse(JSON.parse(fullData));
    ...
    // -> Token Stop Reason
    const tokenStopReason = _fromABOV3StopReason(stop_reason);
    if (tokenStopReason !== null)
      pt.setTokenStopReason(tokenStopReason);
...
function _fromABOV3StopReason(stopReason: ABOV3Wire_API_Message_Create.Response['stop_reason']) {
  switch (stopReason) {
    ...
    default:
      console.warn(`_fromABOV3StopReason: unknown stop reason: ${stopReason}`);
      return null;
  }
}
